...
import { NurseRoomAssignmentDto } from "../../types/nurseTypes";
      ...
      <div className="w-full overflow-x-auto bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
        <table className="min-w-full divide-y divide-white/10">
          ...
          <tbody className="divide-y divide-white/10">
            {nurses.map(nurse => (
              <tr key={nurse.id} className="hover:bg-white/10 transition-colors duration-200">
                <td className="py-4 px-6 text-gray-300">{nurse.id}</td>
                <td className="py-4 px-6 text-gray-300">{`${nurse.firstName} ${nurse.lastName}`}</td>
